...
fn upload_script_and_execute() {
	...
	let deps: Vec<VerifiedModule> = compiled_program.modules.into_iter().map(|m| VerifiedModule::new(m).unwrap()).collect();
	let program = compile_program_with_deps(sender.address(), &program, vec![],
											deps);
