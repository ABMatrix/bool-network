...
use mock::data_store::GENESIS_WRITE_SET;
...
pub struct Executor<T>{
	config: VMConfig,
	data_store: AccessStore<T>,
	p: PhantomData<T>,
}

impl<T: Trait> Executor<T> {
	...
	pub fn from_genesis(
		...
		let mut executor = Executor {
			...
			data_store: AccessStore::default(),
			p: PhantomData,
		};
	...
	pub fn execute_block(&self, txn_block: Vec<SignedTransaction>) -> Vec<TransactionOutput> {
		MoveVM::execute_block(txn_block, &self.config, &self.data_store)
	}
